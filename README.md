...
Перед записью при `--merge`/`--prune` существующий вывод копируется в бэкап
`<out>.bak-<таймстамп>` рядом с выводом (хранятся последние 10, старые удаляются).
Бэкап — только у локальных форматов с `--merge` (tree, bundled, postman-v3, yaak,
insomnia-v5); импорт-только форматы (postman/hoppscotch/insomnia v4/openapi)
локального состояния не имеют, бэкап невозможен.
...
| `--merge` | выкл | Обновить существующий вывод вместо замены (tree, bundled, postman-v3, yaak, insomnia-v5): значения параметров и токены пользователя сохраняются, новые методы добавляются, описания актуализируются, сортировка пересчитывается. Перед записью создаётся бэкап `<out>.bak-<таймстамп>` (последние 10). |

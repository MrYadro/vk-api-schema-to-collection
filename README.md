...
| `generate_collection.py` | Строит коллекцию. Флаги: `--format tree\|bundled\|postman\|openapi`, `--all`, `--api-version`, `--schema-dir`, `--descriptions`, `--out`. |
| `validate_collection.py` | Валидирует сгенерированное по официальным JSON Schema: OpenCollection (tree/bundled), Postman (коллекция/окружение), OpenAPI 3.1. Формат определяется по пути. |
